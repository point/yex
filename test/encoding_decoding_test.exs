defmodule Y.EncodingDecodingTest do
  ...
  test "decode text with formatting from Y.js" do
    # Generated by Y.js (V2 encoding):
    # const doc = new Y.Doc();
    # const text = doc.getText('text');
    # doc.transact(() => {
    #   text.insert(0, 'hello', { bold: true });
    #   text.insert(5, ' world');  // no attributes = inherits surrounding format
    # });
    # Y.encodeStateAsUpdateV2(doc)
    #
    # In Y.js, when inserting without explicit attributes, text inherits the
    # surrounding format. Here " world" is inserted between "hello" and the
    # format end marker (bold: null), so it's considered bold too.
    # The toDelta() output would be: [{"insert":"hello world","attributes":{"bold":true}}]
    #
    # To insert plain text after formatted text, use empty attributes: insert(5, ' world', {})
    msg =
      Base.decode16!(
        "0002000206E4A7DDDF090303000A00010C07060084008600C41D1774657874626F6C6468656C6C6F626F6C6420776F726C64440005040601010000010400787E00",
        case: :mixed
      )

    {:ok, doc} = Doc.new(name: :decode_text_with_formatting)
    ...
    assert "hello world" = Y.Type.Text.to_string(text)

    # Check the items include format markers
    # Order: format-start(bold:true), "hello", " world", format-end(bold:null)
    # Both "hello" and " world" are between the format markers, so both are bold
    items = Y.Type.Text.to_list(text, as_items: true)

    assert [
             %Y.Item{content: [%Y.Content.Format{key: :bold, value: true}]},
             %Y.Item{content: [%Y.Content.String{str: "hello"}]},
             %Y.Item{content: [%Y.Content.String{str: " world"}]},
             %Y.Item{content: [%Y.Content.Format{key: :bold, value: nil}]}
           ] = items
  end
end
